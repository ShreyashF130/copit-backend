...
async def create_customer_order(request: Request):
    ...
    async with db.pool.acquire() as conn:
        order = await conn.fetchrow("""
            SELECT o.total_amount, o.status, s.id as shop_id, s.razorpay_key_id, s.razorpay_key_secret 
        ...
        # 2. Initialize Shop's Razorpay Client with Decrypted Keys
        ...
        try:
            ...
            return {
                ...
                "key_id": decrypted_key_id # ⚠️ MUST RETURN DECRYPTED PUBLIC KEY TO FRONTEND
            }
        except Exception as e:
